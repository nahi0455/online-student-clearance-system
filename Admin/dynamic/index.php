...
// Apply filters
if ($selected_session !== '') { 
    $query_students .= " AND COALESCE(r.session, s.session)='$selected_session'"; 
}
if ($filter == 'approved') { 
    $query_students .= " AND s.is_library_approved = 1"; 
}
elseif ($filter == 'ready') { 
    $query_students .= " AND s.is_library_approved = 0 AND s.is_department_approved = 1"; 
}
elseif ($filter == 'pending') { 
    $query_students .= " AND s.is_library_approved = 0 AND s.is_department_approved = 0"; 
}

$query_students .= " ORDER BY COALESCE(r.fullname, s.fullname) ASC";
$result_students = mysqli_query($conn, $query_students);
...
<head>
<meta charset="utf-8">
<title>Library | <?php echo $dept; ?> Department</title>
    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column">
            ...
            <li class="nav-item">
              <a href="../library/index.php?dept=<?php echo $dept; ?>" class="nav-link">
            ...
            <li class="nav-item">
              <a href="index.php?dept=<?php echo $dept; ?>&view=session" class="nav-link">
...
<li class="nav-item">
  <a href="index.php?dept=<?php echo $dept; ?>&filter=ready" class="nav-link">
...
<li class="nav-item">
  <a href="index.php?dept=<?php echo $dept; ?>&filter=pending" class="nav-link">
...
<li class="nav-item">
  <a href="index.php?dept=<?php echo $dept; ?>&filter=approved" class="nav-link">
...
} elseif (!$row['is_library_approved']) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (sessionSelect) {
    sessionSelect.addEventListener('change', function () {
      const v = this.value;
      const urlBase = 'index.php?dept=<?php echo $dept; ?>&view=session';
