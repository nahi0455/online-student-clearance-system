...
// DEFAULT PHOTO MUST BE SET BEFORE QUERY
$_SESSION['photo'] = "uploads/default.png";

// Get username for DB lookup
$username = $_SESSION['admin-username'];

// Fetch admin info (photo + fullname)
$sql = "SELECT fullname, photo FROM admin WHERE username='$username'";
$result = mysqli_query($conn, $sql);
$row1 = mysqli_fetch_assoc($result);

// If admin not found → use session generated info
if (!$row1) {
    $row1['fullname'] = $_SESSION['fullname'];
    $row1['photo'] = $_SESSION['photo'];
}
...
if ($filter == 'approved') { $query_students .= " AND is_bookstore_approved = 1"; }
elseif ($filter == 'ready') { $query_students .= " AND is_bookstore_approved = 0 AND is_library_approved = 1"; }
elseif ($filter == 'pending') { $query_students .= " AND is_bookstore_approved = 0 AND is_library_approved = 0"; }
...
<head>
<meta charset="utf-8">
<title>bookstore | <?php echo $dept; ?> Department</title>
<link rel="icon" type="image/jpg" sizes="16x16" href="../images/favicon.jpg">
<link rel="stylesheet" href="../plugins/fontawesome-free/css/all.min.css">
<link rel="stylesheet" href="../dist/css/adminlte.min.css">

<style>
/* Modern Bookstore Admin Styling */
:root {
  ...
  --bookstore-gradient: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%);
...
/* Animated bookstore-themed background pattern */
.content-wrapper::before {
  ...
  animation: bookstoreFloat 12s ease-in-out infinite;
  pointer-events: none;
}

@keyframes bookstoreFloat {
  0%, 100% { 
    opacity: 0.4; 
...
/* Enhanced Cards with Bookstore Theme */
.card { 
...
/* Enhanced Table with Bookstore Theme */
.table { 
  color: #1f2937;
  border-radius: var(--border-radius);
  overflow: hidden;
...
/* Enhanced Buttons with Bookstore Theme */
.btn {
...
.btn-info, .view-btn {
  background: var(--bookstore-gradient) !important;
  border: none !important;
  color: white !important;
...
/* Bookstore-specific Enhancements */
.table tbody tr:hover {
...
/* Loading Animation */
.loading-shimmer {
  background: linear-gradient(90deg, rgba(139, 92, 246, 0.1) 25%, rgba(124, 58, 237, 0.2) 50%, rgba(139, 92, 246, 0.1) 75%);
    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column">
            ...
            <li class="nav-item">
              <a href="../bookstore/bookstore.php?dept=<?php echo $dept; ?>" class="nav-link">
            ...
            <li class="nav-item">
              <a href="bookstore.php" class="nav-link">
            ...
            <li class="nav-item">
              <a href="bookstore.php?dept=<?php echo $dept; ?>&view=session" class="nav-link">
...
<li class="nav-item">
  <a href="bookstore.php?dept=<?php echo $dept; ?>&filter=ready" class="nav-link">
...
<li class="nav-item">
  <a href="bookstore.php?dept=<?php echo $dept; ?>&filter=pending" class="nav-link">
...
<li class="nav-item">
  <a href="bookstore.php?dept=<?php echo $dept; ?>&filter=approved" class="nav-link">
...
while($row = mysqli_fetch_assoc($result_students)) {
    ...
    if (empty($student_photo) && !empty($row['photo'])) {
...
} elseif (!$row['is_bookstore_approved']) {
    echo "<form method='POST' action='approve_bookstore.php' style='display:inline-block;'>
            <input type='hidden' name='student_id' value='{$row['ID']}'>
            <button type='submit' name='btnapprove' class='btn btn-success btn-sm'>Approve</button>
          </form>";
} else {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (sessionSelect) {
    sessionSelect.addEventListener('change', function () {
      const v = this.value;
      const urlBase = 'bookstore.php?dept=<?php echo $dept; ?>&view=session';
