...
if ($filter == 'approved') { $query_students .= " AND is_registrar_approved = 1"; }
elseif ($filter == 'ready') { $query_students .= " AND is_registrar_approved = 0 AND is_police_approved = 1"; }
elseif ($filter == 'pending') { $query_students .= " AND is_registrar_approved = 0 AND is_police_approved = 0"; }
...
<head>
<meta charset="utf-8">
<title>registrar Dashboard | <?php echo $dept; ?> Department</title>
    ...
    <div class="sidebar">
      <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column">
            ...
            <li class="nav-item">
              <a href="../registrar/registrar.php?dept=<?php echo $dept; ?>" class="nav-link">
            ...
            <li class="nav-item">
              <a href="registrar.php?dept=<?php echo $dept; ?>&view=session" class="nav-link">
...
<li class="nav-item">
  <a href="registrar.php?dept=<?php echo $dept; ?>&filter=ready" class="nav-link">
...
<li class="nav-item">
  <a href="registrar.php?dept=<?php echo $dept; ?>&filter=pending" class="nav-link">
...
<li class="nav-item">
  <a href="registrar.php?dept=<?php echo $dept; ?>&filter=approved" class="nav-link">
...
while($row = mysqli_fetch_assoc($result_students)) {
   ...
   <a href='leave_details.php?id=".$row['ID']."'>
    ...
    // Approval button
if (!$row['is_police_approved']) {
    echo "<button class='btn btn-warning btn-sm' disabled>
            waiting 
          </button>";
} elseif (!$row['is_registrar_approved']) {
    echo "<form method='POST' action='approve_registrar.php' style='display:inline-block;'>
    ...
    echo "<button class='btn btn-secondary btn-sm' disabled>Cleareds</button>";
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (sessionSelect) {
    sessionSelect.addEventListener('change', function () {
      const v = this.value;
      const urlBase = 'registrar.php?dept=<?php echo $dept; ?>&view=session';
