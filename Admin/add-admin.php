...
// Set default admin session for direct access (no login required)
...
// Optional: Get admin info if exists, but don't require it
...
//save users details
$query = "INSERT INTO `admin` (username,password,fullname,email,status,photo,role,department)
VALUES ('$username','$password','$fullname','$email','Active','uploads/avatar_nick.png','$role','$department')";
...
if ($_SESSION['role'] == 'department_head') {
    echo "You are Department Head of " . $_SESSION['department'];
}
...
<head>
  ...
  <title>Create Admin User | BULEHORA UNIVERSITY</title>
...
<aside class="main-sidebar sidebar-dark-primary elevation-4">
    <a href="index.php" class="brand-link">
      ...
      <span class="brand-text">BULE HORA UNIVERSITY<br><small>Admin Management</small></span>
    </a>
        ...
        <div class="row justify-content-center">
          <div class="col-lg-8 col-md-10 col-12">
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- Enhanced form start -->
               <form id="adminForm" action="" method="post" class="modern-form">
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group">
                        <label for="username"><i class="fas fa-user"></i> Username</label>
                        <input type="text" class="form-control" name="txtusername" id="username" value="<?php if (isset($_POST['txtusername'])) echo $_POST['txtusername']; ?>" placeholder="Enter unique username" required>
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-group">
                        <label for="fullname"><i class="fas fa-id-card"></i> Full Name</label>
                        <input type="text" class="form-control" name="txtfullname" id="fullname" value="<?php if (isset($_POST['txtfullname'])) echo $_POST['txtfullname']; ?>" placeholder="Enter full name" required>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <input type="password" class="form-control" name="txtpassword" id="password" value="<?php if (isset($_POST['txtpassword'])) echo $_POST['txtpassword']; ?>" placeholder="Enter secure password" required>
                    ...
                    <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <input type="password" class="form-control" name="txtpassword2" id="password2" value="<?php if (isset($_POST['txtpassword2'])) echo $_POST['txtpassword2']; ?>" placeholder="Confirm password" required>
                      </div>
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="role"><i class="fas fa-user-tie"></i> User Role</label>
                    <select name="role" id="role" class="form-control" required>
                        <option value="">🎯 Select Administrative Role</option>
                        <option value="department_head">🎓 Department Head</option>
                        <option value="library">📚 Library (Chief of Circulation)</option>
                        <option value="bookstore">📖 University Bookstore</option>
                        <option value="dormitory">🏠 Dormitory Management</option>
                        <option value="cafeteria">🍽️ Student Cafeteria Service</option>
                        <option value="sport">⚽ Sport Master</option>
                        <option value="dean">🎓 Office of the Student Dean</option>
                        <option value="police">👮 Campus Police</option>
                        <option value="registrar">📋 Registrar Office</option>
                        <option value="super_admin">👑 Super Administrator</option>
                        <option value="registr">📋 Registrar student</option>

                    </select>
                    
                    <!-- Role Preview Card -->
                    <div id="rolePreview" style="display:none; margin-top: 15px; padding: 15px; border-radius: 12px; background: linear-gradient(135deg, rgba(0, 123, 255, 0.1), rgba(204, 204, 255, 0.05)); border: 2px solid #007bff; animation: slideIn 0.3s ease;">
                      <div style="display: flex; align-items: center; gap: 15px;">
                        <div id="roleIcon" style="font-size: 40px; animation: bounce 0.5s ease;"></div>
                        <div style="flex: 1;">
                          <div style="font-weight: 700; color: #007bff; font-size: 16px; margin-bottom: 5px;">Selected Role:</div>
                          <div id="roleText" style="font-weight: 600; color: #374151; font-size: 18px;"></div>
                          <div id="roleDescription" style="font-size: 13px; color: #6b7280; margin-top: 5px;"></div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="form-group" id="facultyGroup" style="display:none;">
                    <label for="faculty"><i class="fas fa-university"></i> Faculty / College</label>
                    <select name="faculty" id="faculty" class="form-control">
                        <option value="">🏛️ Select Faculty/College</option>
                  ...
                  <div class="form-group">
                    ...
                    <input type="email" class="form-control" name="txtemail" id="email" value="<?php if (isset($_POST['txtemail'])) echo $_POST['txtemail']; ?>" placeholder="Enter email address" required>
                  </div>


<script>
// Role selection logic with preview
document.getElementById('role').addEventListener('change', function () {
    const role = this.value;
    const isHead = role === 'department_head';
    
    // Show/hide faculty and department groups
    ...
    const roleData = {
        'department_head': {
            ...
            description: 'Manages academic department operations, faculty, and curriculum development'
        },
        'library': {
            icon: '📚',
            name: 'Library (Chief of Circulation)',
            description: 'Oversees library operations, book circulation, and resource management'
        },
        'bookstore': {
            icon: '📖',
            name: 'University Bookstore',
            description: 'Manages bookstore inventory, sales, and student textbook services'
        },
        'dormitory': {
            icon: '🏠',
            name: 'Dormitory Management',
            description: 'Supervises student housing, room assignments, and residential facilities'
        },
        'cafeteria': {
            icon: '🍽️',
            name: 'Student Cafeteria Service',
            description: 'Manages dining services, meal plans, and food quality standards'
        },
        'sport': {
            icon: '⚽',
            name: 'Sport Master',
            description: 'Coordinates athletic programs, sports facilities, and student activities'
        },
        'dean': {
            icon: '🎓',
            name: 'Office of the Student Dean',
            description: 'Handles student affairs, discipline, and academic support services'
        },
        'police': {
            icon: '👮',
            name: 'Campus Police',
            description: 'Ensures campus security, safety protocols, and emergency response'
        },
        'registrar': {
            icon: '📋',
            name: 'Registrar Office',
            description: 'Manages student records, enrollment, grades, and transcripts'
        },
        'super_admin': {
            icon: '👑',
        ...
        'registr': {
            ...
            description: 'Manages student records, registrar, and change pasword,'
       }
    ...
    if (role && roleData[role]) {
        document.getElementById('roleIcon').textContent = roleData[role].icon;
        document.getElementById('roleText').textContent = roleData[role].name;
        document.getElementById('roleDescription').textContent = roleData[role].description;
    ...
    if (department) {
        ...
        departmentPreview.style.display = 'none';
                ...
                <div class="card-footer text-center">
                  <button type="submit" name="btncreate" class="btn btn-primary btn-lg">
