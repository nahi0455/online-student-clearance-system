<?php
 ...
 error_reporting(0);
 include('../connect.php'); 
if(empty($_SESSION['admin-username']) && empty($_SESSION['matric_no']))
    {   
    header("Location: login.php"); 
    }
    else{
    }
    $isStudent = !empty($_SESSION['matric_no']);
    $identifier = $isStudent ? $_SESSION['matric_no'] : $_SESSION['admin-username'];
	
	date_default_timezone_set('Africa/Lagos');
$current_date = date('Y-m-d H:i:s');

$sql = $isStudent ? "select * from register where matric_no='$identifier'" : "select * from admin where username='$identifier'"; 
$result = $conn->query($sql);
$row1= mysqli_fetch_array($result);

  $q = $isStudent ? "select * from register where matric_no = '$identifier'" : "select * from admin where username = '$identifier'";
  $q1 = $conn->query($q);
...
if(isset($_POST["btnpassword"])){
  ...
  <?php } else {
   ...
   if ($isStudent) {
     $sql = "update register set `password`='$confirm_new' where matric_no='".$_SESSION['matric_no']."'";
   } else {
